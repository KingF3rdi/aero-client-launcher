    ...
    <div className="p-8 flex flex-col gap-5 h-full">
      ...
      <div className="flex gap-2">
        {TABS.map((t) => (
          <button
            ...
            onClick={() => setTab(t.id)}
            className={
              "flex items-center gap-2 label-mc flex items-center h-9 px-4 text-[11px] border transition-colors cursor-pointer " +
      ...
      <div className="flex-1 overflow-y-auto grid grid-cols-2 gap-3 content-start">
        ...
        {results.map((item) => (
          <div
            key={item.id}
            className="flex items-center gap-3 bg-black/30 border border-accent/30 backdrop-blur p-4"
          >
            {item.iconUrl ? (
